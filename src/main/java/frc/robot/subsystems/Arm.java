...
public class Arm {
  ...
  public static void moveToPreset() {
    ...
    if (preset == "driving") {
      ...
      desiredHeight = positions.customHeight;
      desiredLength = positions.customLength;
    } else {
  ...
  public static void setExtender(double speed) {
    if (RobotContainer.safteyEnabled) {
      if (lifterEncoder.getPosition() < -80) {
        DriverStation.reportWarning("ARM TOO LOW TO EXTEND!!", false);
        stopExtender();
      } else if (lifterEncoder.getPosition() > -10) {
        ...
        stopExtender();
      } else if ((extenderEncoder.getPosition() < (-5)) && (speed < -5)) {
  ...
  public static void customPosition(double Lift, double Extend) {
    positions.customHeight = Lift;
    positions.customLength = Extend;
    preset = "custom";
    System.out.println("Moving arm to preset: " + preset);
  }
...
class positions {
  ...
  public static final double bottomLength = -3;

  public static final double scoringHeight = -40;
  public static final double scoringLength = 225;
  
  public static double customHeight = 0;
  public static double customLength = 0;
}